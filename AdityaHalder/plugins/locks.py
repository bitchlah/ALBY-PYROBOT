...
async def tg_lock(
    ...
    if lock:
        if perm not in permissions:
            return await message.reply("Sudah terkunci.")
        permissions.remove(perm)
    else:
        if perm in permissions:
            return await message.reply("Sudah Dibuka.")
    ...
    try:
        ...
        return await message.reply(
            "Untuk membuka ini, Anda harus membuka 'pesan' terlebih dahulu."
        )

    await message.reply(("Terkunci." if lock else "Tidak terkunci."))
...
async def locks_func(client: Client, message: Message):
    ...
    if parameter in data:
        ...
        await message.reply(f"🔐 **Mengunci Semuanya di** {message.chat.title}")
        ...
        await message.reply(f"🔓 **Membuka Semuanya di** {message.chat.title}")
...
async def locktypes(client: Client, message: Message):
    ...
    if not permissions:
        return await message.reply("Tidak ada izin.")
...
`.locks` <all atau jenis lock> - **Mengunci izin di grup**

`.unlock` <all atau jenis unlock> - **Membuka izin di grup**
