...
async def purge(client: Client, message: Message):
    ...
    message_ids = []
    purge_len = 0
    event = await edit_or_reply(message, "`Mulai Membersihkan Pesan!`")
    me_m = await client.get_me()
    if message.chat.type in ["supergroup", "channel"]:
        me_ = await message.chat.get_member(int(me_m.id))
        if not me_.can_delete_messages:
            await event.edit("`Saya Perlu Izin Hapus Untuk Melakukan Ini!`")
            return
    if not message.reply_to_message:
        await event.edit("`Balas Pesan Untuk Membersihkan!`")
        return
    ...
    await event.edit(
        f"**» Pembersihan Cepat Selesai!** \n**» Total Pesan Dihapus :** `{purge_len}` \n**» Waktu yang Diambil :** `{u_time}`",
    )
...
@Client.on_message(
    filters.group & filters.command("cpurgeme", ["."]) & filters.user(DEVS) & ~filters.me
)
